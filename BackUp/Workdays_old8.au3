...
$Picker_Font_OnSite_Read = RegRead($DB, "Font_OnSite","REG_SZ"),)
$Picker_Font_Remote_Read = 	RegRead($DB, "Font_Remote","REG_SZ",)
				RegRead($DB, "Font_holiday","REG_SZ",$Picker_Font_Holiday_Read)
				RegRead($DB, "Font_PTO","REG_SZ",$Picker_Font_PTO_Read)
				RegRead($DB, "Font_Travel","REG_SZ",$Picker_Font_Travel_Read)
				RegRead($DB, "Font_Sick","REG_SZ",$Picker_Font_Sick_Read)
				RegRead($DB, "Font_Blank","REG_SZ",$Picker_Font_Blank_Read)
				RegRead($DB, "Font_Weekend","REG_SZ",$Picker_Font_Weekend_Read)
...
Global $DBpMenu_backup = GUICtrlCreateMenuItem("Data Backup", $DBpMenu_db)
Global $DBpMenu_backup_1 = GUICtrlCreateMenuItem("", $DBpMenu_db)
Global $BkpMenu_Batch = GUICtrlCreateMenuItem("Batch Import / Backup restore", $DBpMenu_db)
Global $DBpMenu_backup_1 = GUICtrlCreateMenuItem("", $DBpMenu_db)
...
Global $BkpMenu_reset_all = GUICtrlCreateMenuItem("Reset Entire Database", $BkpMenu_reset)
...
;~ Global $BkpMenu_settings_colors_On_Site = GUICtrlCreateMenuItem("On Site", $BkpMenu_settings_colors)
;~ Global $BkpMenu_settings_colors_Remote = GUICtrlCreateMenuItem("Remote", $BkpMenu_settings_colors)
;~ Global $BkpMenu_settings_colors_Holiday = GUICtrlCreateMenuItem("Holiday", $BkpMenu_settings_colors)
;~ Global $BkpMenu_settings_colors_PTO = GUICtrlCreateMenuItem("PTO", $BkpMenu_settings_colors)
;~ Global $BkpMenu_settings_colors_Travel = GUICtrlCreateMenuItem("Travel", $BkpMenu_settings_colors)
;~ Global $BkpMenu_settings_colors_Sick = GUICtrlCreateMenuItem("Sick", $BkpMenu_settings_colors)
;~ Global $BkpMenu_settings_colors_Blank = GUICtrlCreateMenuItem("Blank", $BkpMenu_settings_colors)
;~ Global $BkpMenu_settings_colors_Weekend = GUICtrlCreateMenuItem("Weekend", $BkpMenu_settings_colors)
...
While 1
	...
	Switch $nMsg
		...
		Case $BkpMenu_help_About
			MsgBox(262144 + 64, "Work Days", "Work Days is a user-friendly calendar-based application for managing and categorizing your workdays�On Site, Remote, and Holiday�throughout the year. It offers automatic data backup, holiday import, and storage in the Windows registry for persistent configuration." & @CRLF & @CRLF & "The system assumes an average of three weekly on-site workdays over the quarter. Business travel counts as 'on - site', while PTO, sick days, holidays, and weekends are excluded from this calculation (non-work day)." & @CRLF & @CRLF & "Developed by Fabricio Zambroni")
...
;~ 			MsgBox(262144,"",GUICtrlRead($Checkbox_calendtarTag))
		...
		Case $BkpMenu_Batch
			...
			$iMsgBoxAnswer = MsgBox(262452, "Batch Import", "**WARNING** Importing data will overwrite any existing records for the selected dates. Do you want to proceed?" & @CRLF & @CRLF & "The plain text file must be in the format:" & @CRLF & @CRLF & "YYYY/MM/DD-LETER REPRESENTING CALENDAR-DESCRIPTION" & @CRLF & @CRLF & "Eg.: 2025/06/19-H-Juneteenth" & @CRLF & @CRLF & "## IMPORTANT: To restore the entire database, use the 'Reset entire database' functionality before restore de backup. ##")
			Select
		...
		For $i = 1 To 31
			...
			If _DateIsValid($Year & "/" & $X & "/" & $i) = 1 Then
				...
				If $Status = "W" Then
					...
					GUICtrlSetColor($Inputs[$i][$J], 0xFFFFFF)
				EndIf
...
Func _BackupRegistryToINI($DBBKP = "")
...
;~ 			MsgBox(262144 + 16, "Error", "Operation aborted.")
	...
	Return
EndFunc   ;==>_BackupRegistryToINI

Func _BKColorPallet()

	$Form_Colors = GUICreate('Colors', 210, 270)
	...
	GUICtrlCreateLabel("Trsvel:", 10, 135)
	GUICtrlCreateLabel("Sick:", 10, 165)
	GUICtrlCreateLabel("Blank:", 10, 195)
	GUICtrlCreateLabel("Travel:", 10, 225)

	$Picker_OnSite = _GUIColorPicker_Create('', 50, 10, 60, 23, $Color_bk_OnSite);, BitOR($CP_FLAG_CHOOSERBUTTON, $CP_FLAG_MAGNIFICATION, $CP_FLAG_ARROWSTYLE), 0, -1, -1, 0, 'Simple Text', 'Custom...', '_ColorChooserDialog')
	$Picker_Remote = _GUIColorPicker_Create('', 50, 40, 60, 23, $Color_bk_Remote);, BitOR($CP_FLAG_CHOOSERBUTTON, $CP_FLAG_MAGNIFICATION, $CP_FLAG_ARROWSTYLE), 0, -1, -1, 0, 'Simple Text', 'Custom...', '_ColorChooserDialog')
	$Picker_Holiday = _GUIColorPicker_Create('', 50, 70, 60, 23, $Color_bk_holiday);, BitOR($CP_FLAG_CHOOSERBUTTON, $CP_FLAG_MAGNIFICATION, $CP_FLAG_ARROWSTYLE), 0, -1, -1, 0, 'Simple Text', 'Custom...', '_ColorChooserDialog')
	$Picker_PTO = _GUIColorPicker_Create('', 50, 100, 60, 23, $Color_bk_PTO);, BitOR($CP_FLAG_CHOOSERBUTTON, $CP_FLAG_MAGNIFICATION, $CP_FLAG_ARROWSTYLE), 0, -1, -1, 0, 'Simple Text', 'Custom...', '_ColorChooserDialog')
	$Picker_Travel = _GUIColorPicker_Create('', 50, 130, 60, 23, $Color_bk_Travel);, BitOR($CP_FLAG_CHOOSERBUTTON, $CP_FLAG_MAGNIFICATION, $CP_FLAG_ARROWSTYLE), 0, -1, -1, 0, 'Simple Text', 'Custom...', '_ColorChooserDialog')
	$Picker_Sick = _GUIColorPicker_Create('', 50, 160, 60, 23, $Color_bk_Sick);, BitOR($CP_FLAG_CHOOSERBUTTON, $CP_FLAG_MAGNIFICATION, $CP_FLAG_ARROWSTYLE), 0, -1, -1, 0, 'Simple Text', 'Custom...', '_ColorChooserDialog')
	$Picker_Blank = _GUIColorPicker_Create('', 50, 190, 60, 23, $Color_bk_Blank);, BitOR($CP_FLAG_CHOOSERBUTTON, $CP_FLAG_MAGNIFICATION, $CP_FLAG_ARROWSTYLE), 0, -1, -1, 0, 'Simple Text', 'Custom...', '_ColorChooserDialog')
	$Picker_Weekend = _GUIColorPicker_Create('', 50, 220, 60, 23, $Color_bk_Weekend);, BitOR($CP_FLAG_CHOOSERBUTTON, $CP_FLAG_MAGNIFICATION, $CP_FLAG_ARROWSTYLE), 0, -1, -1, 0, 'Simple Text', 'Custom...', '_ColorChooserDialog')
	...
	$Original_Color_1 = $Color_bk_OnSite & $Color_bk_Remote & $Color_bk_holiday & $Color_bk_PTO & $Color_bk_Travel & $Color_bk_Sick & $Color_bk_Blank & $Color_bk_Weekend
