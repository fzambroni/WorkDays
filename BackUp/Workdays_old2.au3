...
Func _Update($SelDate)
	...
	If $Data_year & "/" & $Data_month & "/" & $Data_day = @YEAR & "/" & @MON & "/" & @MDAY Then
					...
					GUICtrlSetColor($Inputs[$Data_day][$Data_month], 0xFFCCCC) ; Sick
					GUICtrlSetFont($Inputs[$Data_day][$Data_month], 10, 1200, "", "", 5)

					GUICtrlSetTip($Inputs[$Data_day][$Data_month], $WeekDayName & " - " & $Year & "/" & $X & "/" & $n & " - TODAY" & $tip)
					If $Status = "" Then
						$Status = "X"
						GUICtrlSetColor($Inputs[$Data_day][$Data_month], 0xFF0000) ; today
					EndIf

					GUICtrlSetData($Inputs[$Data_day][$Data_month], "[" & $Status & "]")
