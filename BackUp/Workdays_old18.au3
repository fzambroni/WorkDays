...
Global $About = "1.0.1.3 - Custom colors and bug fixes" & @CRLF & "1.0.1.4 - Code polishing and new custom color palette"
...
;~ 			_ClearScreen()
		...
		Case $Button_OnSite
			...
			MsgBox(262144, "", $CheckDate_Return)
			If $CheckDate_Return <> 0 And $CheckDate_Return <> "O" Then

If Not IsDeclared("iMsgBoxAnswer") Then Local $iMsgBoxAnswer
$iMsgBoxAnswer = MsgBox(262436,"Replace current value","You're about to replace the current status for the selected date. " & @CRLF"" & @CRLF & "Current Status: xx" & @CRLF & "New Status: yy" & @CRLF"" & @CRLF & "Do you want to continue?")
Select
	Case $iMsgBoxAnswer = 6 ;Yes
		$UpdateDate = 1
...
Func _CheckDate($DateToCheck)
	...
	If $DateToCheck_Value <> "" Then
		Return StringLeft($DateToCheck_Value, 1)
	Else
		Return 0
	EndIf
...
Func _Update($SelDate)
	...
	If $Data_year & "/" & $Data_month & "/" & $Data_day = @YEAR & "/" & @MON & "/" & @MDAY Then
		...
		GUICtrlSetFont($Inputs[$Data_day][$Data_month], 10, 900, "", "", 2)
		...
		For $i = 1 To 31
			...
			If _DateIsValid($Year & "/" & $X & "/" & $i) = 1 Then
				...
				If $Year & "/" & $X & "/" & $n = @YEAR & "/" & @MON & "/" & @MDAY Then
					...
					GUICtrlSetFont($Inputs[$i][$J], 10, 900, "", "", 2)
