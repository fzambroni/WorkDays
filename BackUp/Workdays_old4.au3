...
Global $BkpMenu_calendar = GUICtrlCreateMenu("Calendar")
Global $BkpMenu_Batch = GUICtrlCreateMenuItem("Batch Import / Backup restore", $BkpMenu_calendar)
...
$Button_OnSite = GUICtrlCreateButton("&On Site", 296, 54, 75, 25)
GUICtrlSetBkColor($Button_OnSite, 0x00CC66)

$Button_Remote = GUICtrlCreateButton("&Remote", 384, 54, 75, 25)
GUICtrlSetBkColor($Button_Remote, 0x0080FF)
...
$Button_holiday = GUICtrlCreateButton("&Holiday", 296, 94, 75, 25)
GUICtrlSetBkColor($Button_holiday, 0xFFFFCC)

$Button_PTO = GUICtrlCreateButton("&PTO", 384, 94, 75, 25)
GUICtrlSetBkColor($Button_PTO, 0x66FFFF)

$Button_Travel = GUICtrlCreateButton("&Travel", 296, 134, 75, 25)
GUICtrlSetBkColor($Button_Travel, 0xFF8000)

$Button_Sick = GUICtrlCreateButton("&Sick", 384, 134, 75, 25)
GUICtrlSetBkColor($Button_Sick, 0xFF6666)

$Button_Blank = GUICtrlCreateButton("&Blank", 296, 174, 75, 25)


$Button_Weekend = GUICtrlCreateButton("&Weekend", 384, 174, 75, 25)
GUICtrlSetBkColor($Button_Weekend, 0xA0A0A0)
...
;~ $Label_3_q2 = GUICtrlCreateLabel("Ratio:", 856, 70, 69, 21, BitOR($SS_RIGHT,$SS_BLACKFRAME))
...
While 1
	...
	Switch $nMsg
		...
		Case $BkpMenu_help_help
			If Not FileExists(@ScriptDir & "\help.pdf") Then
...
Func _Update($SelDate)
	...
	If $Data_Register = "T" Then GUICtrlSetBkColor($Inputs[$Data_day][$Data_month], 0xFF8000) ; Travel

	If $Data_Register = "W" Then
		GUICtrlSetBkColor($Inputs[$Data_day][$Data_month], 0xA0A0A0)             ; Weekend
		GUICtrlSetColor($Inputs[$Data_day][$Data_month], 0xFFFFFF)
	EndIf

	If $Data_Register = "O" Then GUICtrlSetBkColor($Inputs[$Data_day][$Data_month], 0x00CC66) ; On-site

	If $Data_Register = "R" Then GUICtrlSetBkColor($Inputs[$Data_day][$Data_month], 0x0080FF) ; Remote

	If $Data_Register = "P" Then GUICtrlSetBkColor($Inputs[$Data_day][$Data_month], 0x66FFFF) ; PTO

	If $Data_Register = "H" Then GUICtrlSetBkColor($Inputs[$Data_day][$Data_month], 0xFFFFCC) ; holiday

	If $Data_Register = "S" Then GUICtrlSetBkColor($Inputs[$Data_day][$Data_month], 0xFF6666) ; Sick

	If $Data_Register = "" Then GUICtrlSetBkColor($Inputs[$Data_day][$Data_month], 0xFFFFFF) ; Blank
...
;~ 		$LabelMonthX[$J] = GUICtrlCreateLabel($Return, 8, 216 + $Skip + ($J * 25), 50, 20)
	...
	If $Year = @YEAR Then
		If @MON = "01" Or @MON = "02" Or @MON = "03" Then
			GUICtrlSetData($Input_RT_q1, $Ratio_Q1)
		...
		If @MON = "04" Or @MON = "05" Or @MON = "06" Then
			GUICtrlSetData($Input_RT_q2, $Ratio_Q2)
		...
		If @MON = "07" Or @MON = "08" Or @MON = "09" Then
			GUICtrlSetData($Input_RT_q3, $Ratio_Q3)
		...
		If @MON = "10" Or @MON = "11" Or @MON = "12" Then
			GUICtrlSetData($Input_RT_q4, $Ratio_Q4)
...
;~
		...
		For $i = 1 To 31
			...
			If _DateIsValid($Year & "/" & $X & "/" & $i) = 1 Then
				...
				If $Status = "W" Then
					GUICtrlSetBkColor($Inputs[$i][$J], 0xA0A0A0) ; Weekend
					GUICtrlSetColor($Inputs[$i][$J], 0xFFFFFF)
				EndIf

				If $Status = "O" Then
					GUICtrlSetBkColor($Inputs[$i][$J], 0x00CC66) ; On-site
				EndIf

				If $Status = "R" Then
					GUICtrlSetBkColor($Inputs[$i][$J], 0x0080FF) ; Remote
				EndIf

				If $Status = "T" Then
					GUICtrlSetBkColor($Inputs[$i][$J], 0xFF8000) ; Travel
				EndIf

				If $Status = "P" Then
					GUICtrlSetBkColor($Inputs[$i][$J], 0x66FFFF) ; PTO
				EndIf

				If $Status = "H" Then
					GUICtrlSetBkColor($Inputs[$i][$J], 0xFFFFCC) ; holiday
				EndIf

				If $Status = "S" Then
					GUICtrlSetBkColor($Inputs[$i][$J], 0xFF6666) ; Sick
				EndIf
		...
		For $i = 1 To 31
			...
			If _DateIsValid($Year & "/" & $X & "/" & $i) = 1 Then
				...
				If $Status = "O" Then
					...
					If $J = "07" Or $J = "08" Or $J = "09" Then
;~ 						MsgBox(262144,"$Counta_R_Onsite_q3",$Year & "/" & $X & "/" & $n) ;#####################################
						$Counta_WD_q3 += 1
						$Counta_R_Onsite_q3 += 1
...
Func _CheckQuarter()
	...
	If $SelDate_slipt[1] = @YEAR Then
		...
		If @MON = "01" Or @MON = "02" Or @MON = "03" Then
			GUICtrlSetData($Label_3_q1, "Ratio To Date:")
			GUICtrlSetColor($Label_3_q1, 0x0066CC)
			GUICtrlSetData($Label_3_q2, "Ratio:")
			GUICtrlSetColor($Label_3_q2, $Color_Black)
			GUICtrlSetData($Label_3_q3, "Ratio:")
			GUICtrlSetColor($Label_3_q3, $Color_Black)
			GUICtrlSetData($Label_3_q4, "Ratio:")
			GUICtrlSetColor($Label_3_q4, $Color_Black)
...
;~ 			GUICtrlSetData($Input_RT_q1, $Ratio_Q1)
		...
		If @MON = "04" Or @MON = "05" Or @MON = "06" Then
			...
			GUICtrlSetColor($Label_3_q1, $Color_Black)
			GUICtrlSetData($Label_3_q2, "Ratio To Date:")
			GUICtrlSetColor($Label_3_q2, 0x0066CC)
			GUICtrlSetData($Label_3_q3, "Ratio:")
			GUICtrlSetColor($Label_3_q3, $Color_Black)
			GUICtrlSetData($Label_3_q4, "Ratio:")
			GUICtrlSetColor($Label_3_q4, $Color_Black)
...
;~ 			GUICtrlSetData($Input_RT_q2, $Ratio_Q2)
		...
		If @MON = "07" Or @MON = "08" Or @MON = "09" Then
			GUICtrlSetData($Label_3_q1, "Ratio:")
			GUICtrlSetBkColor($Label_3_q1, $Color_Black)
			GUICtrlSetData($Label_3_q2, "Ratio:")
			GUICtrlSetBkColor($Label_3_q2, $Color_Black)
			GUICtrlSetData($Label_3_q3, "Ratio To Date:")
			GUICtrlSetBkColor($Label_3_q3, 0x0066CC)
			GUICtrlSetData($Label_3_q4, "Ratio:")
			GUICtrlSetBkColor($Label_3_q4, $Color_Black)
...
;~ 			GUICtrlSetData($Input_RT_q3, $Ratio_Q3)
		...
		If @MON = "10" Or @MON = "11" Or @MON = "12" Then
			...
			GUICtrlSetBkColor($Label_3_q2, $Color_Black)
			GUICtrlSetData($Label_3_q3, "Ratio:")
			GUICtrlSetBkColor($Label_3_q3, $Color_Black)
			GUICtrlSetData($Label_3_q4, "Ratio To Date:")
			GUICtrlSetBkColor($Label_3_q4, 0x0066CC)
