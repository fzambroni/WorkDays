...
Func _CheckDate($DateToCheck, $NewStatus )
	...
	If $DateToCheck_Value <> "" Then
		...
		If $CheckDate_Return <> 0 And $CheckDate_Return <> $NewStatus  Then
				If Not IsDeclared("iMsgBoxAnswer") Then Local $iMsgBoxAnswer
				$iMsgBoxAnswer = MsgBox(262436, "Replace current value", "You're about to replace the current status for the selected date. " & @CRLF"" & @CRLF & "Current Status: " & StringLeft($DateToCheck_Value, 1) & @CRLF & "New Status: " & $NewStatus & @CRLF"" & @CRLF & "Do you want to continue?")
				Select
					Case $iMsgBoxAnswer = 6 ;Yes
						Return 0
