...
#AutoIt3Wrapper_Res_Fileversion=1.0.6.9
...
ConsoleWrite($Count & " - $MonitorInfo[0][2]: " & $MonitorInfo[$Count][0] & @CRLF)
ConsoleWrite($Count & " - $MonitorInfo[1][1]: " & $MonitorInfo[$Count][1] & @CRLF)
ConsoleWrite($Count & " - $MonitorInfo[1][2]: " & $MonitorInfo[$Count][2] & @CRLF)
ConsoleWrite($Count & " - Width: " & $MonitorInfo[$Count][3] & @CRLF)
ConsoleWrite($Count & " - Height: " & $MonitorInfo[$Count][4] & @CRLF)
ConsoleWrite($Count & " - IsPrimary: " & $MonitorInfo[$Count][5] & @CRLF)
ConsoleWrite($Count & " - $Window_X: " & $Window_X & @CRLF)
ConsoleWrite($Count & " - $Window_Y: " & $Window_Y & @CRLF)
#ce
$Restart = ""
...
$Button_Update = GUICtrlCreateButton("UPDATE AVAILABLE",296,50,245,30,$SS_CENTER)
GUICtrlSetColor($Button_Update ,0xFF0000)
GUICtrlSetFont($Button_Update ,14,700)
...
;~ 	ConsoleWrite($ValueConsole & @CRLF)
	...
	$ValueConsole = "_Chart() start..."
;~ 	ConsoleWrite($ValueConsole & @CRLF)
	_Chart()
	$ValueConsole = "_Chart() end..."
;~ 	ConsoleWrite($ValueConsole & @CRLF)
...
;~ 	$ContextItem_Date[$U][$V] = GUICtrlCreateMenuItem("Date: " & $XV & "/" & $XU & "/" & $SelDate_slipt[1], $Context[$U][$V])
	$ContextItem_Date[$U][$V] = GUICtrlCreateMenuItem("Date: " & $XV & "/" & $XU & "/" & $SelYear, $Context[$U][$V])
...
;~ 	ConsoleWrite("1271 Returning: " & $i & @CRLF)
...
;~ 	If $Data_Register = "B" Then $StatusName = ""
;~ 	If $Data_Register = "" Then $StatusName = ""
;~ 	If $Data_Register = "   " Then $StatusName = ""
...
Func _ReadINI($iYear, $Splash = 0)
	ConsoleWrite("Inicio do tempo" & @CRLF)
	$Timer = TimerInit()
...
;~ 	_ClearScreen()
	...
	; Top day labels
	For $i = 1 To $iMaxDays
;~ 		$LabelMonth[$i] = GUICtrlCreateLabel($aDayCaption[$i], 5 + ($i * 35), 216, 20, 20, $SS_CENTER)$SS_BLACKFRAME
		$LabelMonth[$i] = GUICtrlCreateLabel($aDayCaption[$i], 5 + ($i * 35), 212, 20, 15, $SS_CENTER)
	Next
	If $Splash = 1 Then
		GUICtrlSetData($Progress_Splash, 60)
		GUICtrlSetData($Label_Percentage, "60%")
;~ 		ConsoleWrite("GUICtrlRead($Button_Close_Splash):" & GUICtrlRead($Button_Close_Splash) & @CRLF)
		If GUICtrlRead($Button_Close_Splash) = $GUI_CHECKED Then
			Exit
		EndIf
	EndIf
...
;~ 		If $sMonthStr = 1 Then $sMonthName = "JAN"
;~ 		If $sMonthStr = 2 Then $sMonthName = "FEB"
;~ 		If $sMonthStr = 3 Then $sMonthName = "MAR"
;~ 		If $sMonthStr = 4 Then $sMonthName = "APR"
;~ 		If $sMonthStr = 5 Then $sMonthName = "MAY"
;~ 		If $sMonthStr = 6 Then $sMonthName = "JUN"
;~ 		If $sMonthStr = 7 Then $sMonthName = "JUL"
;~ 		If $sMonthStr = 8 Then $sMonthName = "AUG"
;~ 		If $sMonthStr = 9 Then $sMonthName = "SEP"
;~ 		If $sMonthStr = 10 Then $sMonthName = "OCT"
;~ 		If $sMonthStr = 11 Then $sMonthName = "NOV"
;~ 		If $sMonthStr = 12 Then $sMonthName = "DEC"

		; Month label on the left
		$LabelMonthX[$j] = GUICtrlCreateLabel($sMonthName, 8, 208 + $Skip + ($j * 25), 20, 20, $SS_CENTER)

		If $Splash = 1 Then
			$countMonthsToSplash += 1
			GUICtrlSetData($Progress_Splash, $countMonthsToSplash)
			GUICtrlSetData($Label_Percentage, $countMonthsToSplash & "%")
;~ 			ConsoleWrite("GUICtrlRead($Button_Close_Splash):" & GUICtrlRead($Button_Close_Splash) & @CRLF)
			If GUICtrlRead($Button_Close_Splash) = $GUI_CHECKED Then
				Exit
			EndIf
...
;~ 					$StatusName = "BLANK"
			...
			$WeekDayName = _DateDayOfWeek($WeekDayNum, 1)
			$WeekNumber = _WeekNumberISO ( $iYear, $sMonthStr, $sDayStr )
...
;~          $WeekNumber = ""
			...
			If $tip <> "" Then
				GUICtrlSetTip($Inputs[$i][$j], StringReplace($tip, @CRLF, @CRLF & "- "), _
						$WeekDayName & " (W" & $WeekNumber & ") - " & $iYear & "/" & $sMonthStr & "/" & $sDayStr & " - " & $StatusName)
			Else
				GUICtrlSetTip($Inputs[$i][$j], $StatusName, _
						$WeekDayName & " (W" & $WeekNumber & ") - " & $iYear & "/" & $sMonthStr & "/" & $sDayStr)
			EndIf
			...
			If $bCurrentYear And ($sMonthStr = @MON) And ($sDayStr = @MDAY) Then
				...
				If $tip <> "" Then
					GUICtrlSetTip($Inputs[$i][$j], StringReplace($tip, @CRLF, @CRLF & "- "), _
							$WeekDayName & " (W" & $WeekNumber & ") - " & $iYear & "/" & $sMonthStr & "/" & $sDayStr & _
							" - " & $StatusName & " - TODAY")
				Else
					GUICtrlSetTip($Inputs[$i][$j], $StatusName, _
							$WeekDayName & " (W" & $WeekNumber & ") - " & $iYear & "/" & $sMonthStr & "/" & $sDayStr & " - TODAY")
				EndIf
		...
		Next ; day loop
		If $Splash = 1 Then
			GUICtrlSetData($Progress_Splash, 75)
			GUICtrlSetData($Label_Percentage, "75%")
			If GUICtrlRead($Button_Close_Splash) = $GUI_CHECKED Then
				Exit
			EndIf
		EndIf
...
;~ 	ConsoleWrite("$XCount: " & $XCount & @CRLF)
	...
	If $Splash = 1 Then
		GUICtrlSetData($Progress_Splash, 77)
		GUICtrlSetData($Label_Percentage, "77%")
		If GUICtrlRead($Button_Close_Splash) = $GUI_CHECKED Then
			Exit
		EndIf
	EndIf
	...
	ConsoleWrite("Time to load: " &  TimerDiff($Timer) & @CRLF)
...
Func _BKColorPallet()
	...
	$Form_Colors = GUICreate('Colors', 230, 440, 300, 100, $DS_MODALFRAME, BitOR($WS_EX_TOPMOST, $WS_EX_MDICHILD), $Form_WorkDays)
...
;~ 	$P_Graphic = GUICtrlCreateButton("x",70,340,30,30)
	...
	$Colors_Close = GUICtrlCreateButton("Close", 85, 370, 70, 30)
...
;~ 				MsgBox(262144, "1", $Color_bk_Graphic & @CRLF & $Picker_Color_Graphic)
			...
			Case $Colors_Close
				...
				#cs
				_GUIColorPicker_Delete($Picker_Color_OnSite)
				_GUIColorPicker_Delete($Picker_Color_Remote)
				_GUIColorPicker_Delete($Picker_Color_Holiday)
				_GUIColorPicker_Delete($Picker_Color_PTO)
				_GUIColorPicker_Delete($Picker_Color_Travel)
				_GUIColorPicker_Delete($Picker_Color_Sick)
				_GUIColorPicker_Delete($Picker_Color_Blank)
				_GUIColorPicker_Delete($Picker_Color_Weekend)
				_GUIColorPicker_Delete($Picker_Color_Today)
				_GUIColorPicker_Delete($Picker_Color_Selected)
				_GUIColorPicker_Delete($Picker_Color_Graphic)
				#ce
