...
Func _EnsureConfig()
	...
	_EnsureRegDefault("Outlook", "IncludeCurrentQuarterStatsInTodayBody", "0")
...
Func _AddRecoveredOutlookItemToMap(ByRef $oOutlookMap, $sDateISO, $oItem, $sStatus)
	...
	Local $sMarker = _GetOutlookMarker($oItem)
...
Func _CreateOrUpdateOutlookItem($oOutlook, $oNs, $sDateISO, $sEntryID, $sStatus, $sMarker)
	...
	$oItem.Body = _BuildOutlookBody($sDateISO, $sMarker)
	...
	_SetUserProp($oItem, "WorkDaysMarker", $sMarker)
	$oItem.Save()
...
Func _EnsureOutlookItemFree($oNs, $sEntryID, $sDateISO, $sStatus, $sMarker)
	...
	If String($oItem.Categories) <> _BuildCategories($sStatus, $sMarker) Then
		...
		$bChanged = True
	EndIf
	Local $sExpectedBody = _BuildOutlookBody($sDateISO, $sMarker)
	If String($oItem.Body) <> $sExpectedBody Then
		$oItem.Body = $sExpectedBody
		$bChanged = True
	EndIf
...
Func _BuildOutlookBody($sDateISO, $sMarker)
	Local $sCleanMarker = StringStripWS(String($sMarker), 2)
	Local $sStats = ""
	If _ShouldIncludeTodayQuarterStats($sDateISO) Then $sStats = _BuildCurrentQuarterStatsText()

	If $sStats = "" Then Return $sCleanMarker

	If $sCleanMarker = "" Then Return $sStats

	Return "Marker:" & @CRLF & $sCleanMarker & @CRLF & @CRLF & $sStats
EndFunc

Func _ShouldIncludeTodayQuarterStats($sDateISO)
	If _Cfg("Outlook", "IncludeCurrentQuarterStatsInTodayBody", "0") <> "1" Then Return False
	If $sDateISO <> _TodayISO() Then Return False
	Return True
EndFunc

Func _BuildCurrentQuarterStatsText()
	Local $aStats = _CurrentQuarterStats()
	If @error Or Not IsArray($aStats) Then Return ""

	Return "WorkDays Quarter Statistics" & @CRLF & _
		"Quarter: " & $aStats[0] & " " & $aStats[1] & @CRLF & _
		"Total Days: " & $aStats[2] & @CRLF & _
		"Estm. On-Site: " & $aStats[3] & @CRLF & _
...
Func _CurrentQuarterStats()
	Local $iYear = Number(@YEAR)
	Local $iMonth = Number(@MON)
	Local $iQuarter = Int(($iMonth - 1) / 3) + 1
	Local $iStartMonth = (($iQuarter - 1) * 3) + 1
	Local $iEndMonth = $iStartMonth + 2

	Local $iTotalDays = 0
	Local $iWorkDays = 0
	Local $iWorkDaysToDate = 0
	Local $iRealOnSite = 0
	Local $iRealOnSiteToDate = 0
	Local $iM, $iD

	For $iM = $iStartMonth To $iEndMonth
		For $iD = 1 To 31
			If _DateIsValid($iYear & "/" & StringFormat("%02d", $iM) & "/" & StringFormat("%02d", $iD)) = 0 Then ContinueLoop
			$iTotalDays += 1

			Local $sStatus = _QuarterStatsReadStatus($iYear, $iM, $iD)
			Local $bToDate = False
			If $iM < Number(@MON) Then
				$bToDate = True
			ElseIf $iM = Number(@MON) And $iD <= Number(@MDAY) Then
				$bToDate = True
			EndIf

			If _QuarterStatsIsWorkDay($sStatus) Then
				$iWorkDays += 1
				If $bToDate Then $iWorkDaysToDate += 1
			EndIf

			If _QuarterStatsIsRealOnSite($sStatus) Then
				$iRealOnSite += 1
				If $bToDate Then $iRealOnSiteToDate += 1
			EndIf
		Next
	Next
...
Func _QuarterStatsReadStatus($iYear, $iMonth, $iDay)
	Local $sValue = RegRead($g_sDB & "\" & $iYear & "\" & StringFormat("%02d", $iMonth), StringFormat("%02d", $iDay))
	If @error Or $sValue = "" Then Return ""
	Return StringUpper(StringLeft(String($sValue), 1))
EndFunc

Func _QuarterStatsIsWorkDay($sStatus)
	Switch StringUpper(String($sStatus))
		Case "O", "R", "T", "", "B"
			Return True
	EndSwitch
	Return False
...
Func _QuarterStatsIsRealOnSite($sStatus)
	Switch StringUpper(String($sStatus))
		Case "O", "T"
			Return True
	EndSwitch
...
Func _SafeQuarterRatio($iRealOnSite, $iWorkDays)
	If Number($iWorkDays) <= 0 Then Return "-"
	If Number($iWorkDays) < 4 Then Return "-"
	Return Round($iRealOnSite / ($iWorkDays / 5), 2)
EndFunc

Func _GetOutlookMarker($oItem)
	If Not IsObj($oItem) Then Return ""

	Local $sBody = String($oItem.Body)
	Local $sMarkerFromBody = _CleanOutlookMarker($sBody)

	; When the generated statistics block is present, the visible body is authoritative.
	; This prevents an old WorkDaysMarker user property from reintroducing a marker
	; after the user has removed it from the Outlook body.
	If StringInStr($sBody, "WorkDays Quarter Statistics") > 0 Then Return $sMarkerFromBody

	If StringStripWS($sMarkerFromBody, 3) <> "" Then Return $sMarkerFromBody

	Local $sMarkerProp = _GetUserProp($oItem, "WorkDaysMarker")
	If $sMarkerProp <> "" Then Return $sMarkerProp
...
Func _CleanOutlookMarker($sBody)
	...
	Local $iStats = StringInStr($s, "WorkDays Quarter Statistics")
	If $iStats > 0 Then
		$s = StringLeft($s, $iStats - 1)
	EndIf

	$s = StringStripWS($s, 2)
	If StringLeft($s, 7) = "Marker:" Then
		$s = StringTrimLeft($s, 7)
	EndIf
...
Func _VLogConfigSnapshot()
	...
	_VLog("Settings snapshot: SubjectPrefix='" & _Cfg("Outlook", "SubjectPrefix", "WorkDays -") & "' CategoryPrefix='" & _Cfg("Outlook", "CategoryPrefix", "WorkDays -") & "' ManagedOnly=" & _Cfg("Outlook", "ManagedOnly", "0") & " DateOrder=" & _Cfg("Outlook", "DateOrder", "Auto") & " IncludeCurrentQuarterStatsInTodayBody=" & _Cfg("Outlook", "IncludeCurrentQuarterStatsInTodayBody", "0") & " VerboseMode=" & _Cfg("Logging", "VerboseMode", "0") & " LogLevel=" & _Cfg("Advanced", "LogLevel", "Normal"))
